    ...
    GT = {
        ...
        ("Hertz", "DE"): {
            ...
            "fields": {
                ...
                "payment_rules": {
                    "expected_value": "Credit and debit cards accepted. EC-Karte not accepted for deposit.",
                    "expected_confidence": "HIGH",
                    "annotation_note": "HERTZ_DE Field 4 — must mention card types (credit/debit). Deposit amounts may also appear.",
                    "check": lambda v: (
                        any(kw in str(v).lower() for kw in ["credit","debit","kreditkarte","debitkarte"]) and
                        not (str(v).lower().strip().startswith("minimum") or
                             str(v).lower().strip().startswith("deposit"))
                    ),
                },
